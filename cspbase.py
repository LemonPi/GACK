...
class Constraint: 
    ...
    def has_support(self, var, val):
        ...
        if (var, val) in self.sup_tuples:
            for t in self.sup_tuples[(var, val)]:
                if self.tuple_is_valid(t): # if any of them are still valid (still in current scope)
                    return True
        return False
    ...
    def tuple_is_valid(self, t):
        ...
        for i, var in enumerate(self.scope): # all variables still have their respective value in the supporting tupple in domain
...
class SoftConstraint: 
    ...
    def __str__(self):
        return("*soft{}({})".format(self.name,[var.name for var in self.scope]))


class SCSP:
    '''Class for packing up a set of variables into a soft CSP problem.
       Contains various utility routines for accessing the problem.
       The variables of the CSP can be added later or on initialization.
       The constraints must be added later'''
    ...
    def add_var(self,v):
        ...
        if not type(v) is Variable:
            ...
            self.vars.append(v)
            self.vars_to_cons[v] = []
            self.vars_to_scons[v] = []

    def add_constraint(self,c):
        '''Add hard constraint to CSP. Note that all variables in the 
           constraints scope must already have been added to the CSP'''
        if not type(c) is Constraint:
            print("Trying to add non constraint ", c, " to CSP object")
        else:
    ...
    def add_soft_constraint(self,c):
        '''Add soft constraint to CSP.'''
        if not type(c) is SoftConstraint:
            print("Trying to add non soft constraint", c, " to SCSP object")
        else:
            for v in c.scope:
    ...
    def get_all_scons(self):
        '''return list of all soft constraints in the SCSP'''
        return self.scons
    ...
    def get_scons_with_var(self, var):
        '''return list of soft constraints that includes var in their scope'''
        return list(self.vars_to_scons[var])
    ...
    def print_all(self):
        print("SCSP", self.name)
        print("   Variables = ", self.vars)
        print("   Constraints = ", self.cons)
        print("   Soft constraints = ", self.scons)


    def print_soln(self):
        print("SCSP", self.name, " Assignments = ")
        for v in self.vars:
            print(v, " = ", v.get_assigned_value(), "    ", end='')
        print("")
...
class BB:
    def __init__(self, csp, UB=float('inf'), LB=0):
        ...
        self.csp = csp
        self.UB = UB # upper bound on solution cost, any path with cost greater than bound gets pruned
        self.LB = LB # return as soon as any solution with cost lower or equal to LB is found (relax optimization requirement)
        self.best_sol = None
    ...
    def bb_search(self,propagator):
        '''Try to solve the soft CSP using specified propagator routine and branch and bound

           propagator == a function with the following template
           propagator(csp, newly_instantiated_variable=None)
           ==> returns (True/False, [(Variable, Value), (Variable, Value) ...]

           csp is a SCSP object---the propagator can use this to get access
           to the variables and constraints of the problem.
        ...
        self.restoreValues(prunings)
        if status == False:
            print("SCSP{} unsolved. Has no solutions".format(self.csp.name))
        if status == True:
    ...
    def bb_recurse(self, propagator, level):
        '''Return true if found solution. False if still need to search.
           If top level returns false--> no solution'''
        ...
        if not self.unasgn_vars:
            ...
            var = self.extractMRVvar()
            if self.TRACE:
                print('  ' * level, "bb_recurse var = ", var)


            for val in self.best_val_order(var):

                if self.TRACE:
                    print('  ' * level, "bb_recurse trying", var, "=", val)

                var.assign(val)
                # prune path here if total cost >= upper bound on solution
                ...
                if self.TRACE:
                    print('  ' * level, "bb_recurse prop status = ", status)
                    print('  ' * level, "bb_recurse prop pruned = ", prunings)

                if status: # not DWO
                    self.bb_recurse(propagator, level+1)
                    # good enough solution found, return without searching for more solutions
                    if self.UB <= self.LB:
                        return

                if self.TRACE:
                    print('  ' * level, "bb_recurse restoring ", prunings)
    ...
    def get_total_cost(self):
        '''total cost of all soft constraints that are assigned'''
        cost = 0
        for c in self.csp.get_all_scons():
            if c.get_n_unasgn() == 0:
                cost += c.get_cost()
        return cost