...
def main():
	...
	for warehouse in range(numWarehouse):
		# warehouse 0, 1, 2 gets cost = 5000
		# 3, 4 gets cost = 10000
		cost = 5000 if warehouse <= numWarehouse // 2 else 10000
		for store in range(numStore):
			constraint = SoftConstraint("channeling" + str(store) + "-" + str(warehouse), [stores[store], warehouses[warehouse]], lambda i, j, middle=warehouse,cost=cost: 0 if not ((i == middle) != False and (j == 1) == False) else cost, mc=cost)
